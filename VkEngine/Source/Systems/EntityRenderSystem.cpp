...
namespace vke
{
	...
	void EntityRenderSystem::CreateShaderAssets(const EngineData& info)
	{
		...
		_instanceBuffers = instancing::CreateStorageBuffers<EntityRenderTask>(info, GetLength());

		// Create descriptor layout.
		jlb::StackArray<layout::Info::Binding, 2> bindings{};
		bindings[0].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		bindings[0].size = sizeof(EntityRenderTask) * GetLength();
		...
		for (size_t i = 0; i < swapChainImageCount; ++i)
		{
			...
			instanceInfo.range = sizeof(EntityRenderTask) * GetLength();
	...
	void EntityRenderSystem::DestroyShaderAssets(const EngineData& info)
	{
		auto& app = *info.app;
		auto& logicalDevice = app.logicalDevice;
		auto& allocator = *info.allocator;
	...
	void EntityRenderSystem::DestroySwapChainAssets(const EngineData& info) const
	{
		auto& logicalDevice = info.app->logicalDevice;
		...
		const jlb::Vector<EntityRenderTask>& tasks)
	{
		...
		assert(!result);
		memcpy(instanceData, static_cast<const void*>(tasks.GetData()), sizeof(EntityRenderTask) * tasks.GetCount());

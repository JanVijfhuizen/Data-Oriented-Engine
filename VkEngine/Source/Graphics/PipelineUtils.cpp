...
namespace vke::pipeline
{
	void Create(const EngineData& engineInfo, const Info& info, VkPipelineLayout& outLayout, VkPipeline& outPipeline)
	{
		...
		modules.Allocate(*engineInfo.tempAllocator, info.modules.length);
		...
		inputAssembly.topology = info.topologyType;
		...
		auto result = vkCreatePipelineLayout(engineInfo.app->logicalDevice, &pipelineLayoutInfo, nullptr, &outLayout);
		assert(!result);
		...
		result = vkCreateGraphicsPipelines(engineInfo.app->logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &outPipeline);
		assert(!result);

		modules.Free(*engineInfo.tempAllocator);
	}
}

...
namespace game
{
	void MovementSystem::Update(const EngineOutData& outData, SystemChain& chain)
	{
		...
		{
			const bool isMoving = !jlb::math::IsZero(task.dir);
			...
			const float&& targetRotation = jlb::math::GetAngle(glm::vec2(0), task.dir);
			auto& rotation = task.transform->rotation;
			const float& rotationDelta = task.rotationSpeed * dt;
			rotation = isMoving ? jlb::math::SmoothAngle(rotation, targetRotation, rotationDelta) : rotation;
		}
		SetCount(0);
	}
}
